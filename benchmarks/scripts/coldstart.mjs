 ...
 * measures two things:
 *   1. "Full process time"    - spawn -> exit. Includes Node/Bun's own boot time.
 *   2. "Import + first call"  - measured inside the process, from import to first App.Path call.
 *
...
printTable({
    ...
    headers: ["Metric", "Baseline (0.1.24)", "Optimized", "Speedup"],
    rows: [
        {
            cells: [
...
const table = [
    ...
    `| Import → first call | ${baseInt.median.toFixed(1)}ms | ${optInt.median.toFixed(1)}ms | **${(baseInt.median / optInt.median).toFixed(2)}x** |`
].join("\n");
